...
export default {
  ...
  methods: {
    setDate() {
      if (
        this.day != "Invalid date" &&
        this.month != "Invalid date" &&
        this.year != "Invalid date"
      ) {
        this.selectedDate = moment(this.year + this.month + this.day).format(
          "YYYY-MM-DD",
        );
    ...
    getDays() {
      if (
        ...
        while (
          moment(this.year + this.month + this.day).format("YYYY-MM-DD") ==
          "Invalid date"
        ) {
    ...
    getYears() {
      ...
      let startyear = "1980";
      while (
        moment(startyear).format("YYYY") <= moment(todayYear).format("YYYY")
      ) {
