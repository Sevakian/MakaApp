<template>
  <div>
    ...
    <div v-for="y in timeline" :key="y.id">
      <h3
        @mouseover="log(y.year, '', '')"
        class="selectable"
        @click="select(y.year, '', '')"
      >
        {{ y.year }}
      </h3>
      <div class="year" :style="isActiveYear == y.year ? activateYear() : 0">
        <div v-for="m in y.months" :key="m.id">
          <div
            class="selectable"
            @click="select(y.year, m.month, '')"
            @mouseover="log(y.year, m.month, '')"
          >
            <span class="monthTitle"> {{ setThisMonth(m.month) }} </span>
            <div class="monthBar" />
          </div>
          <div
            :style="isActiveMonth == y.year + m.month ? activateMonth() : 0"
            class="month"
          >
            <div v-for="d in m.days" :key="d.id">
              <div
                @mouseover="log(y.year, m.month, d.day)"
                class="day"
                @click="select(y.year, m.month, d.day)"
                :style="
                  isActiveDay == y.year + m.month + d.day
                    ? activateDay()
                    : getDayStyle(d)
                "
              ></div>
...
export default {
  ...
  watch: {
    ...
    selectedDay: function () {
      ...
      if (
        this.getSelectedDay == moment(this.getSelectedDay).format("YYYY-MM-DD")
      ) {
