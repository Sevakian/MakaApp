<template>
  <div class="form">
        ...
        <tr>
          ...
          <td>
            <select v-model="dateItem.verwendung">
              <option
                :value="x.verwendung_id"
                v-for="x in verwendungen"
                :key="x.verwendung_id"
              >
                {{ x.text }}
              </option>
    ...
    <div>
      ...
      <button v-if="dateItem.id != ''" v-on:click="save()">Speichern</button>
      <button v-if="dateItem.id != ''" v-on:click="deleteItem()">
        Löschen
      </button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    updateTime() {
      this.dateItem.id = "";
      this.dateItem.date = moment().format("YYYY-MM-DD");
      (this.dateItem.title = ""), (this.dateItem.text = "");
