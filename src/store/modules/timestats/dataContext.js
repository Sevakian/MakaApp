  ...
  mutations: {
    ...
    getDataforDay(state) {
      ...
      if (state.selectedDay == moment(state.selectedDay).format("YYYY")) {
        result = state.allDates.filter(
          (x) => moment(x.context).format("YYYY") == state.selectedDay,
        );
      }
      if (state.selectedDay == moment(state.selectedDay).format("YYYY-MM")) {
        result = state.allDates.filter(
          (x) => moment(x.context).format("YYYY-MM") == state.selectedDay,
        );
      }
      if (state.selectedDay == moment(state.selectedDay).format("YYYY-MM-DD")) {
        result = state.allDates.filter(
          (x) => moment(x.context).format("YYYY-MM-DD") == state.selectedDay,
        );
      }
    ...
    async getAllDates({ commit, dispatch }, inTable) {
      if (inTable != "") {
        ...
        if (result.length == 0) {
          ...
          result = result.sort(
            (a, b) => moment(a.datum).format("X") - moment(b.datum).format("X"),
          );
          for (let i = 0; i < result.length; i++) {
            result[i].datum = moment(result[i].datum).format(
              "YYYY-MM-DD HH:mm:ss",
            );
