    ...
    getGameList: (state) => {
      ...
      if (state.consoleFilter.length != 0) {
        for (let c = 0; c < gamesList_0.length; c++) {
          if (
            gamesList_0[c].consoles.some(
              (val) => state.consoleFilter.indexOf(val) !== -1,
            )
          ) {
      ...
      if (state.valueFilter != "" || state.valueFilter.length != 0) {
        for (let i = 0; i < state.valueFilter.length; i++) {
          for (let j = 0; j < gamesList_1.length; j++) {
            ...
            if (
              gamesList_1[j].Spielename.toLowerCase().includes(
                state.valueFilter[i].toLowerCase(),
              ) ||
              gamesList_1[j].Spieleserie.toLowerCase().includes(
                state.valueFilter[i].toLowerCase(),
              ) ||
              gamesList_1[j].Genre.toLowerCase().includes(
                state.valueFilter[i].toLowerCase(),
              ) ||
              gamesList_1[j].Entwickler.toLowerCase().includes(
                state.valueFilter[i].toLowerCase(),
              )
            ) {
      ...
      if (state.statusFilter.length != 0) {
        for (let j = 0; j < state.statusFilter.length; j++) {
          for (let i = 0; i < gamesList_2.length; i++) {
            ...
            if (
              gamesList_2[i].Status.toLowerCase() ==
              state.statusFilter[j].toLowerCase()
            ) {
  ...
  actions: {
    ...
    orderList({ commit, getters }, inResults) {
      ...
      if (order == "releasedASC") {
        ...
        inResults = inResults.sort(
          (a, b) =>
            moment(b.Erscheinungsjahr).format("X") -
            moment(a.Erscheinungsjahr).format("X"),
        );
      }
      ...
      if (order == "selledUnits") {
        ...
        inResults = inResults.sort(
          (a, b) =>
            parseInt(b.AnzahlVerkauft.replaceAll(".", "")) -
            parseInt(a.AnzahlVerkauft.replaceAll(".", "")),
        );
      }
